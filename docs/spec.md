...
Telegram Mini App для поиска компании на отдых по интересам: карта мероприятий,
подключение к участникам, чаты по мероприятиям, рейтинги, симпатии за звёзды,
PRO-подписка.

## 1. Концепция

Пользователь открывает карту, видит мероприятия рядом (по категориям: спорт,
прогулки, настолки, барбекю, концерты, путешествия и т.д.). Может:
- создать своё мероприятие с точкой на карте, временем и условиями участия
- присоединиться к уже существующему (запрос → подтверждение организатором,
  либо мгновенное вступление, если открытое)

Когда участники собраны — автоматически создаётся чат мероприятия. После
...
### Карта
- полноэкранная карта, метки мероприятий с иконкой категории
- мероприятия "сейчас" визуально выделены (пульсация/кольцо)
- фильтр-чипы категорий сверху (горизонтальный скролл)
- нижняя карточка ближайшего/выбранного мероприятия с превью
- кнопка геолокации
...
- закрытые/приватные мероприятия по инвайту
- больше одновременных активных мероприятий (у free — лимит 1-2)
- расширенная статистика профиля
- кастомный бейдж/рамка аватара
- отмена/редактирование мероприятия без штрафа к рейтингу
...
- жалобы и блокировка прямо из чата мероприятия
