...
export default function VenueChatScreen() {
  ...
  useEffect(() => {
    apiFetch(`/venues/inquiries/${inquiryId}/messages`)
  ...
  useEffect(() => {
    ...
    socket.emit('join_venue_chat', inquiryId)
    socket.on('new_message', (message) => {
      setMessages(prev => (prev ?? []).concat(message))
    })

    return () => {
      socket.emit('leave_venue_chat', inquiryId)
      socket.disconnect()
    }
  }, [status, inquiryId])
  ...
  async function handleSend() {
    ...
    try {
      await apiFetch(`/venues/inquiries/${inquiryId}/messages`, {
        method: 'POST',
  ...
  return (
    <div className="page-full" style={{ display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '16px 16px 8px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        <BackButton />
        <h1 style={{ fontSize: 18, fontWeight: 800 }}>Чат щодо локації</h1>
      </div>
