...
function CategoryForm({ initial, onSave, onCancel, saving }) {
  ...
  return (
    <div className="card" style={{ padding: 14, marginBottom: 10 }}>
      <input
        placeholder="Назва типу закладу" value={name}
...
export default function AdminVenueCategoriesScreen() {
  ...
  function load() {
    apiFetch('/admin/venue-categories')
      .then(({ venue_categories }) => setCategories(venue_categories))
      .catch(err => console.error('[AdminVenueCategories] failed to load:', err.message))
  }
  ...
  async function handleCreate(payload) {
    setSaving(true)
    try {
      await apiFetch('/admin/venue-categories', { method: 'POST', body: JSON.stringify(payload) })
      setCreating(false)
      load()
    } catch (err) {
      console.error('[AdminVenueCategories] create failed:', err.message)
    } finally {
      setSaving(false)
    }
  }
  ...
  async function handleUpdate(id, payload) {
    setSaving(true)
    try {
      await apiFetch(`/admin/venue-categories/${id}`, { method: 'PATCH', body: JSON.stringify(payload) })
      setEditingId(null)
      load()
    } catch (err) {
      console.error('[AdminVenueCategories] update failed:', err.message)
    } finally {
      setSaving(false)
    }
  }
  ...
  async function toggleActive(cat) {
    try {
      await apiFetch(`/admin/venue-categories/${cat.id}`, { method: 'PATCH', body: JSON.stringify({ is_active: !cat.is_active }) })
      load()
    } catch (err) {
      console.error('[AdminVenueCategories] toggle failed:', err.message)
    }
  }
  ...
  return (
    <div className="page">
      <div style={{ padding: '16px 16px 8px', display: 'flex', alignItems: 'center', gap: 12 }}>
        <BackButton />
        <h1 style={{ fontSize: 18, fontWeight: 800 }}>Категорії партнерів</h1>
      </div>
